...
This quickstart consists of a basic hasura project with a simple  swift-perfect app running on it. Once this project is deployed on to a hasura cluster, you will have the perfect app running at https://api.cluster-name.hasura-app.io

This is the right place to start if you are planning to build or want to learn to build a swift-perfect server with hasura.
...
3. A basic swift-perfect app which runs on the `api` subdomain.
...
$ cd hello-swift-perfect
```

The above command does the following:
1. Creates a new folder in the current working directory called `hello-swift-perfect`
2. Creates a new trial hasura cluster for you and sets that cluster as the default cluster for this project
3. Initializes `hello-swift-perfect` as a git repository and adds the necessary git remotes.
...
This quickstart comes with one such custom service written in `swift` using the `perfect` framework. Check it out in action at `https://api.cluster-name.hasura-app.io` . Currently, it just returns a "Hello World" at that endpoint.
